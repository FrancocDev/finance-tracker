...
export default function Dashboard() {
  ...
  const addIncome = async (data) => {
    if (user) {
      const doc = await databases.createDocument(
        DATABASE_ID, COLLECTIONS.INCOMES, ID.unique(),
        { ...data, userId: user.$id }
      );
      setIncomes(prev => [doc, ...prev]);
    } else {
  ...
  const addExpense = async (data) => {
    if (user) {
      const doc = await databases.createDocument(
        DATABASE_ID, COLLECTIONS.EXPENSES, ID.unique(),
        { ...data, userId: user.$id }
      );
    ...
    if (user) {
      ...
      if (existing) {
        await databases.updateDocument(
          DATABASE_ID, COLLECTIONS.BUDGETS, existing.$id,
          { amount: data.amount }
        );
        setBudgets(prev => prev.map(b => b.$id === existing.$id ? { ...b, amount: data.amount } : b));
      } else {
        const doc = await databases.createDocument(
          DATABASE_ID, COLLECTIONS.BUDGETS, ID.unique(),
          { ...data, userId: user.$id }
        );
        setBudgets(prev => [...prev, doc]);
      }
    } else {
    ...
    try {
      ...
      for (const item of local.budgets) {
        const existing = (await databases.listDocuments(
          DATABASE_ID, COLLECTIONS.BUDGETS,
          [Query.equal("userId", currentUser.$id), Query.equal("month", item.month)]
        )).documents[0];
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 16px" }}>
      ...
      <header style={headerStyle}>
        <div>
          ...
          <p style={subtitleStyle}>
            {user ? `Welcome back, ${user.name || user.email}` : "Track your finances locally — save to the cloud anytime"}
          </p>
        </div>
        {user ? (
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => { setAuthMode("login"); setShowAuth(true); }} style={loginButtonStyle}>
              Sign In
            </button>
            <button onClick={() => { setAuthMode("register"); setShowAuth(true); }} style={saveButtonStyle}>
              ☁️ Save to Cloud
            </button>
          </div>
        )}
      ...
      {/* CTA Banner for non-logged users with local data */}
      {hasLocalData && (
        <div style={ctaBannerStyle}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => { setAuthMode("login"); setShowAuth(true); }} style={ctaSecondaryButtonStyle}>
              Sign In
            </button>
            <button onClick={() => { setAuthMode("register"); setShowAuth(true); }} style={ctaButtonStyle}>
              Save to Cloud
            </button>
          </div>
        </div>
      ...
      {migrating && (
        <div style={overlayStyle}>
          <div style={overlayContentStyle}>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#64748b" }}>Please don't close this tab.</p>
          </div>
        </div>
      )}
      ...
      {showAuth && (
        <div style={modalOverlayStyle} onClick={() => setShowAuth(false)}>
          <div style={modalStyle} onClick={e => e.stopPropagation()}>
            <button onClick={() => setShowAuth(false)} style={modalCloseStyle}>×</button>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <div style={{ fontSize: 40, marginBottom: 8 }}>☁️</div>
              <h2 style={{ margin: 0, fontSize: 20 }}>{authMode === "login" ? "Sign In" : "Create Account"}</h2>
              <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: 14 }}>
                {authMode === "login" ? "Access your data from anywhere" : "Start tracking in the cloud"}
              </p>
            </div>
      ...
      {currentBudget && (
        <div style={budgetBarContainerStyle}>
          <div style={budgetBarHeaderStyle}>
            ...
            <span style={{ fontWeight: 700, color: budgetRemaining >= 0 ? "#10b981" : "#ef4444" }}>
